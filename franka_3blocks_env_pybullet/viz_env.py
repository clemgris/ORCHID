...
env = Franka3BlocksEnvPyBullet(cfg, headless=True)
...
if task_id == 3:
    ...
    up_cube, bottom_cube = random.sample(["A", "B", "C"], k=2)

    print(f"Stacking cube {up_cube} on cube {bottom_cube}")
...
os.makedirs("debug_toy_env", exist_ok=True)
imgs[0].save(
    os.path.join("debug_toy_env", f"example_{TASK_NAMES[task_id]}.gif"),
    format="GIF",
