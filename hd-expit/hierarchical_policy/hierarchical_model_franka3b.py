...
from controller.lerobot.common.policies.act.modeling_act import (
    ACTConfig,
    ACTPolicy,
)
from controller.lerobot.common.policies.diffusion.modeling_diffusion import (
    DiffusionConfig,
    DiffusionPolicy,
)
from planner.goal_diffusion import GoalGaussianDiffusion, Trainer
from planner.tools import save_images
...
class HierarchicalModel(CalvinBaseModel):
    def __init__(self, cfg):
        ...
        # Transforms
        self._init_tranforms(cfg.policy.dataset.transforms)
        ...
        self.without_guidance = self.cfg.policy.dataset.get("without_guidance", False)
        ...
        # Vision encoder
        self.use_feat = (
            self.cfg.policy.dataset.goal != "pixel"
            or self.cfg.policy.dataset.obs != "pixel"
        )
        if self.use_feat:
            self._init_vision_encoder(cfg)

        # Normlisation
        self.norm_feat = self.cfg.policy.dataset.norm_feat
        self.feat_stats = None

    def _load_stats(self, cfg):
        # Low level
        stats_path = os.path.join(cfg.policy.stats_path)
    ...
    def _init_vision_encoder(self, cfg):
        # if "dino_vit" in cfg.policy.dataset.goal:
        #     self.vision_encoder = ViTEncoder()
        # elif "r3m" in cfg.policy.dataset.goal:
    ...
    def _init_high_level(self, cfg):
        if cfg.high_level.dataset.goal == "pixel":
            self.high_level_channels = 3
            feature_decoder = None
            target_size = (96, 96)
            channel_mult = (1, 2, 3, 4, 5)
        elif "dino" in cfg.high_level.dataset.goal:
            feature_decoder = None
            self.high_level_channels = 768
            target_size = (
                cfg.high_level.dataset.feat_patch_size,
                cfg.high_level.dataset.feat_patch_size,
            )
            if cfg.high_level.dataset.feat_patch_size == 16:
                channel_mult = (1, 2, 3)
            elif cfg.high_level.dataset.feat_patch_size == 32:
                channel_mult = (1, 2, 3, 4)
            elif cfg.high_level.dataset.feat_patch_size == 64:
                channel_mult = (1, 2, 3, 4, 5)
    ...
    def _init_tranforms(self, transforms_dict):
        ...
        self.transforms = {
            cam: [
                ...
                for transform in transforms_dict.val[cam]
            ]
            for cam in transforms_dict.val
        }
        self.transforms = {
    ...
    def _extract_obs(
        ...
        obs: dict,
        idx: int | np.ndarray,
        space: str = "pixel" or "feat",
        transform: bool = True,
    ):
        if space == "feat":
            raise NotImplementedError("Feature extraction not implemented yet")
        # Apply transform
        o = obs["pixels"][idx].to(self.device)
        if transform:
            o = o * 255
            transformed_o = self.transforms["rgb_static"](o)
        else:
            transformed_o = o
    ...
    def step(self, obs, text_goal, oracle_subgoals=None, encoded_text=None):
            ...
            if sample_subgoals:
                ...
                self.sub_goals = (
                    self.high_level.sample(
                        self.init_subgoal_gen, [text_goal], 1, self.guidance_weight
                    )
                    .cpu()
                    .detach()
                )
        ...
        if self.steps % self.sample_action_every == 0:
            ...
            init = self._extract_obs(obs, 0, self.cfg.policy.dataset.obs)[
                None, None, ...
            ]
            ...
            if self.cfg.policy.dataset.obs == self.cfg.policy.dataset.goal:
