...
class GoalGaussianDiffusion(nn.Module):
    ...
    def model_predictions(
        ...
        if guidance_weight > 0.0:
            uncond_model_output = self.model(
                torch.cat([x, x_cond], dim=1), t, task_embed * 0.0
            )
        ...
        if self.objective == "pred_noise":
            if guidance_weight == 0:
                ...
                pred_noise = (
                    1 + guidance_weight
                ) * model_output - guidance_weight * uncond_model_output
            ...
            if guidance_weight == 0:
                ...
                pred_noise = (
                    1 + guidance_weight
                ) * cond_noise - guidance_weight * uncond_noise
                x_start = self.predict_start_from_noise(x, t, pred_noise)
    ...
    def p_mean_variance(
        self, x, t, x_cond, task_embed, clip_denoised=False, guidance_weight=0
    ):
        preds = self.model_predictions(
            x, t, x_cond, task_embed, guidance_weight=guidance_weight
        )
    ...
    def __init__(
        ...
        *,
        ...
        self.accelerator = Accelerator(
            split_batches=split_batches, mixed_precision=precision
        )
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    goal_embed = (
                        ...
                        * (
                            torch.rand(
                                goal_embed.shape[0], 1, 1, device=goal_embed.device
                            )
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            batches = num_to_groups(
                                self.num_samples, self.valid_batch_size
                            )
                            ...
                            with self.accelerator.autocast():
                                all_xs_list = list(
                                    map(
                                        lambda n, c, e: self.ema.ema_model.sample(
                                            batch_size=n, x_cond=c, task_embed=e
                                        ),
                                        batches,
                                        x_conds,
                                        task_embeds,
