...
from controller.lerobot.common.policies.diffusion.configuration_diffusion import (
    DiffusionConfig,
)
...
from franka_3blocks_env_pybullet.dataset.dataset import Franka3BlocksActionDataset
...
def main(args):
    ...
    for path in ft_data_path:
        train_set = Franka3BlocksActionDataset(path, cfg.dataset)
    ...
    if obs == "pixel":
        ...
        raise NotImplementedError(f"Observation type {obs} not implemented yet.")

    # Goal representation shape
    if goal == "pixel":
        goal_shape = [n_channels, 96, 96]
    else:
        raise NotImplementedError(f"Goal type {goal} not implemented yet.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r", "--results_folder", type=str, default="../results_LL_franka3b_ft/calvin"
