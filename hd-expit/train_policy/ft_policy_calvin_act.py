...
from controller.lerobot.common.policies.act.configuration_act import ACTConfig
from controller.lerobot.common.policies.act.modeling_act import ACTPolicy
...
def main(args):
    ...
    if obs == "pixel":
        ...
        raise NotImplementedError(
            f"Observation type {obs} not implemented for diffusion policy."
        )
    ...
    if goal == "pixel":
        ...
        raise NotImplementedError(
            f"Goal type {goal} not implemented for diffusion policy."
        )
    ...
    if obs == "pixel":
        act_cfg["input_shapes"]["observation.image_static"] = obs_shape
        if args.use_gripper:
            act_cfg["input_shapes"]["observation.image_gripper"] = obs_shape
    else:
        act_cfg["input_shapes"]["observation.feat_static"] = obs_shape
        if args.use_gripper:
            raise NotImplementedError(
    ...
    if goal == "pixel":
        act_cfg["input_shapes"]["observation.image_goal_static"] = goal_shape
        if args.use_gripper:
            act_cfg["input_shapes"]["observation.image_goal_gripper"] = goal_shape
    else:
        act_cfg["input_shapes"]["observation.feat_goal_static"] = goal_shape
        if args.use_gripper:
            raise NotImplementedError(
                "Gripper features are not implemented for diffusion policy."
            )

    act_cfg = ACTConfig(**act_cfg)
    ...
    policy = ACTPolicy(act_cfg, dataset_stats=train_stats_dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r", "--results_folder", type=str, default="../results_ACT_LL_calvin_ft/calvin"
