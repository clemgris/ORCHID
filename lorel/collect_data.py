...
FLAGS = flags.FLAGS
flags.DEFINE_string("savepath", "data/generated_data/", "Path to save the HDF5 dataset")
...
def main(argv):
    ...
    sim_data.create_dataset(
        "ims", (forward_ep, steps_per_ep, imsize, imsize, 3), dtype="f"
    )
    sim_data.create_dataset(
        "actions", (forward_ep, steps_per_ep, action_space), dtype="f"
    )
    ...
    for ep in trange(forward_ep):
        ...
        d_d, d_f = [], []
        d_w = get_motion_desc_sawyer(
            st0[9:11], st[9:11], "white mug"
        )  # White mug motion description
        d_b = get_motion_desc_sawyer(
            st0[11:13], st[11:13], "black mug"
        )  # Black mug motion description
