...
class HierarchicalModel:
    def __init__(self, cfg, transforms_dict=None):
        ...
        # Policy
        self.sample_action_every = int(np.ceil(21 / cfg.num_subgoals)) - 1
        self._init_policy(cfg)

        # High level
        self.replan = cfg.replan
        self.ref_traj_length = 20
        self.guidance_weight = 3
        self.sample_subgoals_every = 4
    ...
    def _load_stats(self, cfg):
        # Low level
        stats_path = os.path.join(cfg.policy.root, "../dataset_stats.pkl")
    ...
    def apply_transform(
        ...
        return transforms(episode)

    def _extract_obs(
        self, obs: dict, idx: int | np.ndarray, space: str = "pixel" or "feat"
    ):
        if space == "pixel":
            image = self.apply_transform(obs, self.transforms["pixel"])

        else:
            # Apply transforms for feature observations
            image_obs = self.apply_transform(
                obs,
                self.transforms["feat"],
            )
            if image_obs.ndim == 3:
                image = image_obs[None, ...]
        ...
        return image.to(dtype=torch.float32, device=self.device)
    ...
    def step(self, obs, text_goal, oracle_subgoals=None):
        ...
        if self.use_oracle_subgoals:
            ...
            self.init_subgoal_gen = self._extract_obs(
                obs,
                0,
                "pixel",  # self.cfg.policy.datamodule.lang_dataset.goal
            )[None]
            ...
            if sample_subgoals:
                print(
                    f"Trial {self.steps // 20}: generating subgoals for '{text_goal}'"
                )
                self.sub_goals = (
